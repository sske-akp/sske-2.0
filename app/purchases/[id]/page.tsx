"use client";

import React from "react";
import { useParams } from "next/navigation";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { usePurchaseDetail } from "@/hooks/purchasesHooks";
import { Printer, ArrowLeft } from "lucide-react";
import Link from "next/link";

const formatINR = (value: number) =>
  new Intl.NumberFormat("en-IN", {
    style: "currency",
    currency: "INR",
  }).format(value);

export default function PurchaseDetailPage() {
  const params = useParams();
  const idParam = params.id as string;
  const batchIds = React.useMemo(() => idParam.split(","), [idParam]);
  const { data: items, isLoading, isError } = usePurchaseDetail(batchIds);

  const handlePrint = () => {
    window.print();
  };

  if (isLoading) {
    return (
      <div className="p-10 text-center text-muted-foreground">
        Loading purchase details...
      </div>
    );
  }

  if (isError || !items || items.length === 0) {
    return (
      <div className="p-10 text-center text-destructive">
        Failed to load purchase details.
      </div>
    );
  }

  const supplierName = items[0].supplierName;
  const purchaseDate = items[0].purchaseDate;
  const status = items[0].status;

  const subtotal = items.reduce((sum, item) => sum + item.totalPrice, 0);
  const gst = subtotal * 0.18;
  const total = subtotal + gst;

  const formattedDate = purchaseDate
    ? new Intl.DateTimeFormat("en-IN", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      }).format(new Date(purchaseDate))
    : "-";

  return (
    <>
      {/* Action bar -- hidden when printing */}
      <div className="p-4 flex items-center gap-3 print:hidden">
        <Link href="/purchases/all">
          <Button variant="outline" size="sm">
            <ArrowLeft className="h-4 w-4 mr-1" />
            Back to Purchases
          </Button>
        </Link>
        <Button size="sm" onClick={handlePrint}>
          <Printer className="h-4 w-4 mr-1" />
          Print Purchase
        </Button>
      </div>

      {/* Purchase content -- print-friendly */}
      <div className="max-w-3xl mx-auto p-8 bg-white text-black print:p-0 print:max-w-none">
        {/* Header */}
        <div className="flex justify-between items-start mb-8">
          <div>
            <h1 className="text-2xl font-bold">SSKE Electricals</h1>
            <p className="text-sm text-gray-500">Electrical Supply & Services</p>
          </div>
          <div className="text-right">
            <h2 className="text-xl font-bold">PURCHASE</h2>
            <p className="text-sm text-gray-500">{formattedDate}</p>
            <p className="text-sm">
              Status:{" "}
              <span className="font-medium capitalize">{status}</span>
            </p>
          </div>
        </div>

        <Separator className="mb-6 print:border-black" />

        {/* Supplier details */}
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <h3 className="text-xs font-semibold uppercase text-gray-400 mb-1">
              Supplier
            </h3>
            <p className="font-medium">{supplierName}</p>
          </div>
        </div>

        {/* Line items table */}
        <table className="w-full mb-6 text-sm">
          <thead>
            <tr className="border-b-2 border-gray-300">
              <th className="text-left py-2 font-semibold">#</th>
              <th className="text-left py-2 font-semibold">Product</th>
              <th className="text-right py-2 font-semibold">Qty</th>
              <th className="text-right py-2 font-semibold">Rate</th>
              <th className="text-right py-2 font-semibold">Amount</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, i) => (
              <tr key={item.id} className="border-b border-gray-200">
                <td className="py-2 text-gray-500">{i + 1}</td>
                <td className="py-2">{item.productName}</td>
                <td className="py-2 text-right">{item.quantity}</td>
                <td className="py-2 text-right">{formatINR(item.pricePerUnit)}</td>
                <td className="py-2 text-right">{formatINR(item.totalPrice)}</td>
              </tr>
            ))}
          </tbody>
        </table>

        {/* Totals */}
        <div className="flex justify-end">
          <div className="w-64 space-y-2">
            <div className="flex justify-between text-sm">
              <span>Subtotal</span>
              <span>{formatINR(subtotal)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>GST (18%)</span>
              <span>{formatINR(gst)}</span>
            </div>
            <Separator className="print:border-black" />
            <div className="flex justify-between font-bold text-lg">
              <span>Total</span>
              <span>{formatINR(total)}</span>
            </div>
          </div>
        </div>

        {/* Footer */}
        <div className="mt-12 pt-6 border-t border-gray-200 text-center text-xs text-gray-400">
          <p>SSKE Electricals | Electrical Supply & Services</p>
        </div>
      </div>
    </>
  );
}
